...
export class NoteEventCollectionBoxAdapter implements BoxAdapter, SelectableLocator<NoteEventBoxAdapter, ppqn, int> {
    ...
    * getActiveStack(position: ppqn, flags: int): Generator<NoteEvent> {
        if (!Bits.every(flags, BlockFlag.transporting)) {return}
        if (!this.#computedExtremas) {this.#computeExtremas()}
        for (const event of this.#events.iterateFrom(position - this.#maxDuration)) {
            if (event.position > position) {return}
            if (position < event.complete) {
                yield event // TODO delta?
            }
        }
    }
