import {Arrays, Circle, clamp, Geom, isDefined, Lifecycle, ObservableValue, ValueMapping} from "std"
...
import {deferNextFrame} from "dom"
...
export const Tape = ({lifecycle, project, tracks}: Construct) => {
    ...
    const headerUpdater = deferNextFrame(() => {
        const pulse = project.position.getValue()
    ...
    lifecycle.own(project.position.subscribe(observer))
    observer(project.position)
