import {Generators, int, Mutable, Notifier, Observer, Option, Selection, Terminable} from "opendaw-std"
...
export class NoteCreateModifier implements NoteModifier {
    ...
    selectedModifyStrategy(): NoteModifyStrategy {
        return {
            ...
            iterateRange: (events: EventCollection<UINoteEvent>, from: number, to: number): Iterable<UINoteEvent> => {
                return Generators.flatten(events.iterateRange(from, to), [this.#creation])
            }
        }
    }
