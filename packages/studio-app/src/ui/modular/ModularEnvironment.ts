...
import {ModuleAdapter, Modules} from "studio-adapters"
import {Address, Vertex} from "lib-box"
import {Direction, ModuleConnectorAdapter} from "studio-adapters"
import {Pointers} from "studio-enums"
import {ModuleConnectionBox} from "studio-boxes"
import {ModuleConnectionAdapter} from "studio-adapters"
...
export class ModularEnvironment implements Terminable {
    ...
    constructor(project: Project, modularAdapter: ModularAdapter, camera: Camera) {
        this.#project = project
        this.#modularAdapter = modularAdapter
        this.#camera = camera

        this.#terminator = new Terminator()
        this.#selection = this.#terminator.own(project.selection.createFilteredSelection(Modules.isVertexOfModule, {
            fx: (adapter: ModuleAdapter) => adapter.box,
            fy: vertex => Modules.adapterFor(project.boxAdapters, vertex.box)
        }))
    ...
    beginRewiring(connection: ModuleConnectionAdapter, connectorAdapter: ModuleConnectorAdapter<any, any>, pointer: Point) {
        ...
        this.#project.editing.modify(() => connection.box.delete())
    ...
    #connect(adapter: ModuleConnectorAdapter<any, any>, other: ModuleConnectorAdapter<any, any>): void {
        ...
        this.#project.editing.modify(() => ModuleConnectionBox.create(this.#project.boxGraph, UUID.generate(), box => {
