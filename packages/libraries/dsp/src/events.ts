...
class EventArrayImpl<E extends Event> implements Omit<EventArray<E>, "iterateRange"> {
    ...
    contains(event: E): boolean {
        ...
        const startIndex: int = BinarySearch.leftMostMapped(this.#array, key, NumberComparator, PositionExtractor)
    ...
    floorLastIndex(position: int): int {
        if (this.#unsorted) {this.#sort()}
        return BinarySearch.rightMostMapped(this.#array, position, NumberComparator, PositionExtractor)
    }

    ceilFirstIndex(position: int): int {
        if (this.#unsorted) {this.#sort()}
        return BinarySearch.leftMostMapped(this.#array, position, NumberComparator, PositionExtractor)
    }
