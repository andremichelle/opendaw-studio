...
export namespace BoxIO {
    ...
    export const create = <K extends keyof TypeMap, V extends TypeMap[K]>(
        					name: K, graph: BoxGraph<TypeMap>, uuid: UUID.Format, constructor?: Procedure<V>): V => {
              				switch (name) {case "NetworkBox": return NetworkBox.create(graph, uuid, constructor as Procedure<NetworkBox>) as V
        case "AudioConnectionBox": return AudioConnectionBox.create(graph, uuid, constructor as Procedure<AudioConnectionBox>) as V
        case "DrumBox": return DrumBox.create(graph, uuid, constructor as Procedure<DrumBox>) as V
        case "DelayBox": return DelayBox.create(graph, uuid, constructor as Procedure<DelayBox>) as V
